...
import {
  getPublicationGuideUpsell,
  getLockedOccasionBadge,
} from "../services/accessCopy";
...
const GUIDES: Record<SupportedPlatform, PublicationGuide> = {
  instagram: {
    ...
    steps: [
      ...
      },
      {
        ...
        title: "CTA de cierre",
        description:
          "Termina el caption con este llamado a la acción para maximizar la interacción:",
        inject: "callToAction",
      },
      {
        icon: "✅",
  ...
  facebook: {
    ...
    frequencySuggestion:
      "3–5 veces por semana en páginas, 1–2 en perfil personal",
    steps: [
      {
        icon: "💻",
      ...
      },
      {
        ...
        description:
          "Pega los hashtags al final del texto para mejorar el descubrimiento:",
        inject: "hashtags",
      },
      {
        icon: "🌐",
      ...
      },
      {
        ...
        title: "CTA de cierre",
        description:
          "Asegúrate de incluir este llamado a la acción al final del post:",
        inject: "callToAction",
      },
      {
        icon: "✅",
  ...
  linkedin: {
    ...
    frequencySuggestion:
      "3–5 veces por semana es suficiente para mantener visibilidad",
    steps: [
      {
        icon: "💼",
  ...
  telegram: {
    ...
    frequencySuggestion:
      "1–2 publicaciones diarias en canales, sin límite predefinido en grupos",
    steps: [
      {
        icon: "📲",
        title: "Abre Telegram",
    ...
    .map((step, i) => {
      const injectedValue =
        step.inject === "hashtags" && post.hashtags
          ? post.hashtags.join(" ")
          : step.inject
            ? ((post[step.inject as keyof StructuredPost] as
                | string
                | undefined) ?? "")
            : "";
      const contentBlock =
          ...
          <div className="text-left">
            ...
            {!isPremium && (
              <div className="text-xs text-slate-400 dark:text-slate-500">
                Guía paso a paso ·{" "}
                {isGuest ? "Disponible al crear tu cuenta" : "Solo Premium"}
              </div>
            )}
          </div>
          {!isPremium && (
