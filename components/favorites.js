...
router.get("/", authenticate, async (req, res) => {
  try {
    const favorites = await Favorite.find({ userId: req.userId }).sort({ timestamp: -1 });
    res.json(favorites);
  } catch (error) {
...
router.post("/", authenticate, async (req, res) => {
  try {
    ...
    if (existing) {
      return res.status(400).json({ error: "Este mensaje ya está en tus favoritos" });
    }

    const favorite = new Favorite({ userId: req.userId, content, occasion, tone });
    await favorite.save();
