...
const OccasionPage: React.FC = () => {
  ...
  if (!rawOccasion || !localized) {
    return (
      <div className="p-20 text-center animate-pulse">
        ...
        <div className="font-bold text-slate-400 italic">Cargando inspiración...</div>
      </div>
    );
  }
  ...
  const relatedOccasions = OCCASIONS.filter(o => {
    if (rawOccasion.slug === 'no-me-dejes-en-visto') return ['perdoname', 'amor', 'anniversary'].includes(o.slug);
    if (rawOccasion.slug === 'perdoname') return ['no-me-dejes-en-visto', 'anniversary'].includes(o.slug);
    if (rawOccasion.slug === 'amor') return ['no-me-dejes-en-visto', 'anniversary', 'cumpleanos'].includes(o.slug);
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 animate-fade-in-up">
      ...
      <header className="mb-12 relative">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-8 mb-8">
          <div className="relative group">
            ...
            <div className={`relative bg-white shadow-xl border border-slate-100 w-20 h-20 md:w-24 md:h-24 rounded-[2rem] flex items-center justify-center transform -rotate-3 hover:rotate-0 transition-all cursor-default ${isVisto ? 'text-green-600 shadow-green-500/10' : 'shadow-blue-500/10'}`}>
              <OccasionIcon slug={rawOccasion.slug} icon={rawOccasion.icon} isLarge />
            </div>
          </div>
        ...
        <div className="relative pl-8 md:pl-10">
          <div className={`absolute left-0 top-0 bottom-0 w-1.5 rounded-full ${isVisto ? 'bg-green-500' : 'bg-gradient-to-b from-blue-500 to-indigo-600'}`}></div>
          <p className="text-xl md:text-2xl text-slate-600 font-light leading-relaxed max-w-4xl italic">
            {localized.description} <span className="text-slate-400 font-normal not-italic">— Elige a quién escribes y el tono para crear un mensaje inolvidable.</span>
          </p>
        </div>
      </header>
      ...
      <section className="mt-24">
        <div className="flex items-center gap-6 mb-10">
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Explora más categorías</h2>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <span className="block text-[10px] uppercase tracking-widest text-slate-400 mb-0.5 group-hover:text-blue-400 transition-colors italic">Ver más</span>
