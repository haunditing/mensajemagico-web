...
import { cancelSubscription, getSubscriptionStatus, reactivateSubscription } from '../services/paymentService';
...
const ProfilePage: React.FC = () => {
  ...
  const handleCancel = async () => {
    if (!user) return;
    if (!window.confirm('¿Estás seguro de que quieres cancelar tu suscripción? Perderás los beneficios al final del periodo actual.')) {
      return;
    }
    ...
    try {
      ...
      showToast('Suscripción cancelada correctamente. El acceso continuará hasta el final del periodo.', 'success');
  ...
  if (!user) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center px-4 text-center animate-fade-in-up">
        ...
        <h2 className="text-2xl font-bold text-slate-900 mb-2">Inicia sesión para ver tu perfil</h2>
        <Link to="/login" className="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 transition-colors mt-4">
          Iniciar Sesión
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12 animate-fade-in-up">
      ...
      <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm mb-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-slate-500 capitalize font-medium">Plan {user.planLevel}</p>
          </div>
        </div>

        <div className="border-t border-slate-100 pt-6">
          <h3 className="text-lg font-bold text-slate-900 mb-4">Estado de la Suscripción</h3>
          ...
          {loading ? (
            ...
            <div className="space-y-4 bg-slate-50 p-5 rounded-xl border border-slate-100">
              <div className="flex justify-between items-center">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${subscription.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-slate-200 text-slate-600'}`}>
              ...
              {subscription.cancelAtPeriodEnd ? (
                 <div className="bg-amber-50 p-4 rounded-lg border border-amber-100">
                   <p className="text-amber-800 text-sm mb-3">
                     ⚠️ Tu suscripción se cancelará automáticamente el <strong>{new Date(subscription.renewalDate).toLocaleDateString()}</strong>. Tienes acceso Premium hasta entonces.
                   </p>
                   <button 
                ...
                <>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-600 font-medium">Próxima renovación</span>
                    <span className="font-bold text-slate-800">{new Date(subscription.renewalDate).toLocaleDateString()}</span>
                  </div>
            ...
            <div className="bg-green-50 p-5 rounded-xl border border-green-100 text-center">
              <p className="text-green-800 font-bold mb-2">✨ Plan Premium Activo</p>
              <p className="text-green-700 text-sm">
                Disfrutas de todos los beneficios Premium.
                <br />
                <span className="text-xs opacity-80">
                  (No se encontró información de facturación recurrente o hubo un error al cargarla)
                </span>
              </p>
            </div>
          ) : (
      ...
      <div className="text-center">
        <button onClick={logout} className="text-slate-400 font-bold hover:text-red-500 transition-colors text-sm">
          Cerrar Sesión
        </button>
      </div>
    </div>
