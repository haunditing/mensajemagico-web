...
const SettingsPage: React.FC = () => {
  ...
  const handleResetTour = () => {
    resetTour("onboarding_tour");
    showToast("Tour reiniciado. Redirigiendo al inicio...", "success");
    navigate("/");
  };
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        <div className="p-6 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-6 border-t border-slate-100 dark:border-slate-700">
            <div>
              <h3 className="font-bold text-slate-800 dark:text-slate-200">Tutorial de Bienvenida</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 max-w-md mt-1">
                ¿Quieres volver a ver la guía paso a paso?
              </p>
            </div>
            <button
              onClick={handleResetTour}
              className="w-full sm:w-auto px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-lg text-sm font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
            >
              Reiniciar Tour
            </button>
          </div>
