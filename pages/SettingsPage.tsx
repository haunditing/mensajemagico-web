...
const SettingsPage: React.FC = () => {
  ...
  if (!user) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center px-4 text-center animate-fade-in-up">
        ...
        <Link to="/login" className="text-blue-600 font-bold hover:underline">Inicia sesión para configurar tu cuenta</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12 animate-fade-in-up">
      <h1 className="text-3xl font-black text-slate-900 mb-8">Configuración</h1>

      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100">
          <h2 className="text-lg font-bold text-slate-900 mb-1">Notificaciones</h2>
          <p className="text-slate-500 text-sm">Gestiona cómo y cuándo te contactamos.</p>
        </div>
        ...
        <div className="p-6 space-y-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-bold text-slate-800">Recordatorios en el navegador</h3>
          ...
          <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
            <div className="flex items-center gap-3 mb-2">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Estado del Navegador</span>
              {browserPermission === "granted" && <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-bold">Permitido ✅</span>}
              {browserPermission === "denied" && <span className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full font-bold">Bloqueado 🚫</span>}
              {browserPermission === "default" && <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-bold">Preguntar ❓</span>}
            </div>
