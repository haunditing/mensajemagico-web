...
const RelationPage: React.FC = () => {
  ...
  if (!rawOccasion || !relationship || !localized) {
    return <div className="p-20 text-center font-bold italic text-slate-400">Cargando contexto...</div>;
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 animate-fade-in-up">
      <nav className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-8 flex items-center gap-3">
        ...
        <Link to={`/mensajes/${rawOccasion.slug}`} className="hover:text-blue-600">{localized.name}</Link>
        <span className="w-1 h-1 bg-slate-300 rounded-full"></span>
      ...
      <header className="mb-12">
        ...
        <h1 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tight leading-[1.1] mb-6">
          Mensajes de {localized.name} para mi <span className="text-gradient">{relationship.label}</span>
        </h1>
        <p className="text-xl md:text-2xl text-slate-600 font-light leading-relaxed max-w-3xl">
          Específicamente diseñados para sorprender a tu {relationship.label.toLowerCase()} en {localized.name}. 
          Encuentra frases profundas, divertidas o cortas listas para compartir.
        </p>
      </header>

      <AdBanner position="top" />

      <Generator occasion={{ ...rawOccasion, name: localized.name }} initialRelationship={relationship} />

      <section className="mt-20 bg-slate-900 rounded-[3rem] p-10 md:p-16 text-white relative overflow-hidden">
        <div className="relative z-10">
          <h2 className="text-3xl font-bold mb-6">Más para tu {relationship.label.toLowerCase()}</h2>
          <p className="text-slate-400 mb-10 max-w-xl text-lg">Cualquier momento es perfecto para recordar cuánto te importa. Explora otras ocasiones populares.</p>
